#This is a slapdash implementation of the validation script for the salted project.
#Be adviced that inp.species and possibly other variables are taken from the original calculation
#and may not be correct for the validation set!
...
def build():
    ...
    species, lmax, nmax, lmax_max, nnmax, ndata, atomic_symbols, natoms, natmax = read_system(osp.join(valPAth, "valid.xyz"))
    ...
    for iconf in testrange:
        ...
        variance += var
        print(f"{iconf+1:d} {(np.sqrt(error/var)*100):<3.3f}", file=efile)
