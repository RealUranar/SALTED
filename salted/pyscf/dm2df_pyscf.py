...
def main(geom_indexes: Union[List[int], None], num_threads: int = None):
    ...
    for cal_idx, (geom_idx, geom) in enumerate(zip(geom_indexes, geoms)):
        ...
        np.save(osp.join(inp.system.saltedpath,inp.qm.path2qm, "coefficients", f"coefficients_conf{geom_idx}.npy"), reordered_data["coef"])
        np.save(osp.join(inp.system.saltedpath,inp.qm.path2qm, "projections", f"projections_conf{geom_idx}.npy"), reordered_data["proj"])
        np.save(osp.join(inp.system.saltedpath,inp.qm.path2qm, "overlaps", f"overlap_conf{geom_idx}.npy"), reordered_data["over"])
